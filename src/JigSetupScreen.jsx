...
function FixtureImageSection({ draft, set }) {
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, alignItems:'start' }}>
      ...
      {/* Left: image */}
      <div>
        <div className="kicker" style={{ color:c.hi, marginBottom:8 }}>FIXTURE IMAGE · ภาพ Fixture</div>
        <div style={{ display:'flex', gap:8, marginBottom:8, alignItems:'center', flexWrap:'wrap' }}>
          ...
          {draft.setupImage && (
            <button onClick={()=>{ setAddMode(a=>!a); cancelForm(); }} style={{
              padding:'5px 12px', background:addMode?c.hi:'#fff', color:c.ink,
              border:`1.5px solid ${addMode?c.hi:c.ink}`, fontFamily:'JetBrains Mono', fontSize:10, fontWeight:700, cursor:'pointer',
      ...
      <div>
        ...
        {cpForm && (
          <div style={{ background:'#fff', border:`2px solid ${c.hi}`, padding:10, marginBottom:10 }}>
            ...
            {cpForm.type==='attribute' && (
              <div style={{ padding:'6px 8px', background:'#f0fdf4', border:`1px solid ${c.ok}`, fontSize:10, fontFamily:'JetBrains Mono', color:c.ok, marginTop:4 }}>ตรวจสอบแบบ OK / NG</div>
            )}
        ...
        {cps.map((cp,i) => (
          <div key={cp.id} onClick={()=>{ setEditIdx(i); setCpForm({...cp}); setPending(null); }} style={{
            display:'flex', gap:8, alignItems:'flex-start', padding:'8px 10px', marginBottom:4,
            background:editIdx===i?'#fff5f0':'#fff', border:`1.5px solid ${editIdx===i?c.hi:c.line}`, cursor:'pointer',
          }}>
...
export default function JigSetupScreen({ jigList, onBack }) {
  ...
  useEffect(() => {
    if (!hasApi()) return;
    apiLoadAll()
      .then(apiData => { setSetup(local => ({ ...local, ...apiData })); setApiMode('sql'); })
  ...
  const doSave = async (jigId) => {
    ...
    setSetup(next); saveSetupLocal(next);
    setExpId(null); setDraft(null);
    if (hasApi()) {
      setApiStatus('saving');
      try { await apiSave(jigId, payload); setApiStatus('saved'); }
  ...
  const doDelete = async (jigId, jigName) => {
    ...
    if (hasApi()) {
      setApiStatus('saving');
      try { await apiDelete(jigId); setApiStatus('saved'); }
      catch (_) { setApiStatus('error'); }
      setTimeout(() => setApiStatus(null), 3000);
    }
  };
  ...
  const exportSetup = () => {
    ...
    a.href = url; a.download = 'jig-setup.json'; a.click();
    URL.revokeObjectURL(url);
  };
  ...
  const onImport = (e) => {
    ...
    reader.onload = async (ev) => {
      try {
        ...
        const next = { ...setup };
        for (const item of arr) { if (item.id) next[item.id] = { ...next[item.id], ...item }; }
        setSetup(next); saveSetupLocal(next);
        if (hasApi()) {
          setApiStatus('saving');
          try { await Promise.all(arr.filter(i=>i.id).map(i=>apiSave(i.id,i))); setApiStatus('saved'); }
          catch (_) { setApiStatus('error'); }
          setTimeout(() => setApiStatus(null), 3000);
        }
        alert(`นำเข้าสำเร็จ · ${arr.length} รายการ`);
      } catch (_) { alert('ไฟล์ไม่ถูกต้อง'); }
    };
  ...
  return (
    <div style={{ minHeight:'100vh', background:c.paper, fontFamily:'Inter, system-ui' }}>
      ...
      <div style={{ background:c.ink, color:'#fff', padding:'12px 18px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <div>
          <div className="kicker" style={{ color:c.hi }}>JIG SETUP · LINE 061</div>
      ...
      <div className="screen-pad" style={{ maxWidth:1200, margin:'0 auto' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginBottom:16 }}>
          ...
          <KPI label="ACTIVE"          sub="ใช้งาน"            value={`${activeCount} / ${enriched.length}`}       accent={c.ok} />
          <KPI label="SETUP COMPLETE"  sub="กรอกข้อมูลแล้ว"   value={`${setupCount} / ${enriched.length}`}        accent={c.hi} />
        </div>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11 }}>
            <thead>
              <tr style={{ background:c.paper, borderBottom:`1.5px solid ${c.ink}` }}>
                {['JIG ID','NAME · ชื่อ','PROCESS','PART NO','S/N','LOCATION','PM CYCLE','STATUS','',''].map(h => (
            ...
            <tbody>
              {filtered.flatMap(j => {
                ...
                const rows = [
                  <tr key={j.id} style={{
                    ...
                    <td style={{ padding:'8px 6px' }}>
                      {hasSetup && (
                        <button onClick={()=>doDelete(j.id,j.name)} title="รีเซ็ตข้อมูล" style={{
                ...
                if (isExpanded && draft) {
                  rows.push(
                    <tr key={`${j.id}-edit`} style={{ borderBottom:`1px solid ${c.line}` }}>
                      <td colSpan={10} style={{ padding:0 }}>
                        <div style={{ padding:20, background:'#fff8f5', borderTop:`3px solid ${c.hi}` }}>
                          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
                            <div className="kicker" style={{ color:c.hi }}>SETUP · {j.id} — {j.name}</div>
                          ...
                          <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:10, marginBottom:20 }}>
                            ...
                            <Field label="LOCATION">
                              <input value={draft.location} placeholder="LINE 061"
                          ...
                          <div style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:10, marginBottom:20, alignItems:'start' }}>
                            <Field label="NOTES · หมายเหตุ">
                              <textarea value={draft.setupNote} rows={2} placeholder="ข้อควรระวัง / ขั้นตอนพิเศษ..."
                          ...
                          <div style={{ display:'flex', gap:8, justifyContent:'space-between', marginTop:20, alignItems:'center' }}>
                            ...
                            <div style={{ display:'flex', gap:8 }}>
                              ...
                              <button onClick={()=>doSave(j.id)} style={{ padding:'8px 24px', background:c.ok, color:'#fff', border:'none', fontFamily:'JetBrains Mono', fontSize:11, fontWeight:700, cursor:'pointer' }}>✓ SAVE</button>
                            </div>
                          </div>
                        </div>
                      </td>
