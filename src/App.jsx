...
const c = {
  ...
  okBg:'#e7f3e8', ngBg:'#fbe7e6', okBorder:'#2f7d32', ngBorder:'#c8201d'
};
const S = {
  app:{ minHeight:'100vh', background:c.bg, color:c.text, fontFamily:"'Inter', 'Sarabun', system-ui, sans-serif" },
  hdr:{ background:'#0d0d0d', borderBottom:`3px solid ${c.accent}`, padding:'10px 18px', display:'flex', justifyContent:'space-between', alignItems:'center', color:'#fff' },
  ...
  inp:{ width:'100%', background:'#fff', border:`1.5px solid ${c.text}`, borderRadius:0, padding:'6px 8px', color:c.text, fontSize:11, fontFamily:"'JetBrains Mono', monospace" },
  sel:{ width:'100%', background:'#fff', border:`1.5px solid ${c.text}`, borderRadius:0, padding:'6px 8px', color:c.text, fontSize:11, fontFamily:"'JetBrains Mono', monospace" },
  lbl:{ fontSize:10, color:c.muted, marginBottom:3, display:'block', fontFamily:"'JetBrains Mono', monospace", fontWeight:700, letterSpacing:'0.06em', textTransform:'uppercase' },
  tag:(v)=>({ display:'inline-block', padding:'2px 8px', borderRadius:2, fontSize:10, fontWeight:700, fontFamily:"'JetBrains Mono', monospace", letterSpacing:'0.06em',
    background:v==='OK'?c.okBg:v==='NG'?c.ngBg:'#f4f1ea',
    color:v==='OK'?c.ok:v==='NG'?c.ng:c.muted,
    border:`1.5px solid ${v==='OK'?c.okBorder:v==='NG'?c.ngBorder:c.border}` }),
  th:{ background:'#0d0d0d', color:'#ff6a00', padding:'5px 7px', textAlign:'center', border:`1px solid #0d0d0d`, fontSize:9, fontWeight:700, fontFamily:"'JetBrains Mono', monospace", letterSpacing:'0.06em', textTransform:'uppercase' },
  td:(ng=false)=>({ padding:'4px 7px', border:`1px solid ${ng?c.ngBorder:c.border}`, textAlign:'center', color:ng?c.ng:c.text, fontSize:10.5, background:ng?c.ngBg:'#fff', fontFamily:"'JetBrains Mono', monospace" }),
  secHdr:(ng=false)=>({ background:ng?c.ng:'#0d0d0d', padding:'8px 14px', fontSize:11, fontWeight:700, color: ng ? '#fff' : '#ff6a00', letterSpacing:'0.08em', textTransform:'uppercase', cursor:'pointer', display:'flex', justifyContent:'space-between', alignItems:'center', borderRadius:0, fontFamily:"'JetBrains Mono', monospace" }),
};
    ...
    <div style={S.app}>
      <div style={S.hdr}>
        ...
        <div style={{display:'flex',gap:8,alignItems:'center'}}>
          ...
          {onNavigate && session && ['supervisor','admin'].includes(session.role) && (
            <button style={S.btn('#ff6a00')} onClick={()=>onNavigate('dashboard')}>📊 Dashboard</button>
          )}
          {onNavigate && <button style={S.btn('#334155')} onClick={()=>onNavigate('calibration')}>🔧 Cal</button>}
          {onNavigate && session?.role==='admin' && <button style={S.btn('#334155')} onClick={()=>onNavigate('admin')}>⚙ Admin</button>}
          {session && (
            <span style={{fontSize:9,color:'#cbd5e1',padding:'4px 8px',background:'#1e293b',borderRadius:3}}>
              {session.name} · {session.role} · S{session.shift}
            </span>
          )}
          {onLogout && <button style={S.btn('#7f1d1d')} onClick={onLogout}>↩ Sign Out</button>}
        </div>
      </div>
      <div style={S.wrap}>
          ...
          <div key={grp.label} style={S.card}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))',gap:8}}>
              {grp.jigs.map(jig=>{
                ...
                return (
                  <div key={jig.id} onClick={()=>onOpen(jig)} style={{background:'#fff',border:`1.5px solid ${isNG?c.ngBorder:c.text}`,borderRadius:0,cursor:'pointer',overflow:'hidden',transition:'border 0.2s',display:'flex',flexDirection:'column'}}>
        ...
        {jig.sections.map(sec=>{
          ...
          return (
            <div key={sec.id} style={{marginBottom:8}}>
              ...
              {isOpen && (
                <div style={{background:'#fff',border:`1.5px solid ${c.text}`,borderTop:'none',padding:10,borderRadius:0,overflowX:'auto'}}>
