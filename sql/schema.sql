...
CREATE TABLE IF NOT EXISTS jig_check_sections (
  section_id      VARCHAR(64) NOT NULL,
  jig_id          VARCHAR(64) NOT NULL REFERENCES jig_fixtures(jig_id) ON DELETE CASCADE,
  section_title   VARCHAR(255) NOT NULL,
  check_type      VARCHAR(32) NOT NULL CHECK (check_type IN ('locatepin_simple', 'locatepin_xy', 'feeler', 'checklist', 'su', 'normal', 'uni', 'grip')),
  standard_type   VARCHAR(32) NOT NULL CHECK (standard_type IN ('attribute', 'dimension', 'range', 'diameter')),
  standard_text   TEXT,
  sort_order      INTEGER NOT NULL DEFAULT 0,
  PRIMARY KEY (jig_id, section_id)
);

CREATE TABLE IF NOT EXISTS jig_check_points (
  point_id        VARCHAR(64) NOT NULL,
  jig_id          VARCHAR(64) NOT NULL,
  section_id      VARCHAR(64) NOT NULL,
  point_label     VARCHAR(255) NOT NULL,
  standard_type   VARCHAR(32) NOT NULL CHECK (standard_type IN ('attribute', 'dimension', 'range', 'diameter')),
  nominal_value   NUMERIC(10, 3),
  min_value       NUMERIC(10, 3),
  max_value       NUMERIC(10, 3),
  diameter_value  NUMERIC(10, 3),
  unit_code       VARCHAR(16),
  standard_text   TEXT,
  sort_order      INTEGER NOT NULL DEFAULT 0,
  PRIMARY KEY (jig_id, section_id, point_id),
  FOREIGN KEY (jig_id, section_id) REFERENCES jig_check_sections(jig_id, section_id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_jig_check_points_type ON jig_check_points(jig_id, standard_type);
